...
class OrderPageFirsAction(BasePage):
    ...
    def first_name_field_input(self, first_name):
        return self.find_element_by_locator(OrderPageFirstLocator.FIRST_NAME_FIELD).send_keys(first_name)

    @allure.step('Ввести в поле "Фамилия" {last_name}')
    def last_name_field_input(self, last_name):
        return self.find_element_by_locator(OrderPageFirstLocator.LAST_NAME_FIELD).send_keys(last_name)

    @allure.step('Ввести в поле "Адрес" {address}')
    def address_field_input(self, address):
        return self.find_element_by_locator(OrderPageFirstLocator.ADDRESS_FIELD).send_keys(address)
    ...
    def metro_station_choice(self, metro_station):
        ...
        self.find_element_by_locator(OrderPageFirstLocator.METRO_STATION_INPUT).click()
    ...
    def phone_number_field_input(self, phone_number):
        return self.find_element_by_locator(OrderPageFirstLocator.PHONE_NUMBER_FIELD).send_keys(phone_number)

    @allure.step('Кликнуть кнопку "Далее"')
    def click_next_button(self):
        return self.find_element_by_locator(OrderPageFirstLocator.NEXT_BUTTON).click()
